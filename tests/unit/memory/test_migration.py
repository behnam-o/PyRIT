...
@pytest.mark.parametrize(
    ...
    [
        (
            {"row_id": "attack-1", "field_name": "operator", "value_type": 2, "value_size": 10},
            "non-string labels.operator",
        ),
        (
            {"row_id": "attack-2", "field_name": "operation", "value_type": 1, "value_size": 258},
...
def test_scored_expectation_mssql_backfill_uses_set_based_update(function_name: str, query_name: str):
    ...
    with (
        patch.object(mig.op, "get_bind", return_value=connection),
        patch.object(mig, "_report_progress") as report_progress,
    ):
        getattr(mig, function_name)()

    connection.exec_driver_sql.assert_called_once_with(getattr(mig, query_name))
